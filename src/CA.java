...
public class CA { 
	...
	boolean [] negatives = new boolean[1024];
	...
	public boolean execute(int opCode, int destination, byte r1, int r2Immediate, int index) throws Exception {
		...
		case 3:
			if(negatives[index])
				registerFile[destination] = (byte) (-1*r2Immediate);
			else 	
	...
	public void run() throws Exception {
		...
		for (int clockCycle = 0; clockCycle < (instructionMemory.length + 2); clockCycle++) {
			...
			if (!execute.isEmpty()) {
				...
				try {
					branch = execute((int) arr[0], (int) arr[1], (byte) arr[2], (int) arr[3], clockCycle-2);
				} catch (ClassCastException c) {
					branch = execute((int) arr[0], (int) arr[1], (byte) arr[2], (byte) arr[3], clockCycle-2);
				}
			}
			if (!decode.isEmpty()) {
				if(branch){
	...
	public void interpreter(String filePath) throws Exception {
		...
		while (br.ready()){
			...
			case "ldi":
				if(Integer.parseInt(tokens.get(2))<0)
	...
	public static String verifyNumber(String num)throws Exception {
		...
		int number;
		if(num.charAt(0)=='-'){
			String positive = num.substring(1);
			int pstv = Integer.parseInt(positive);
			String res = Integer.toBinaryString(pstv);
			for(int i = res.length() ; i < 6 ; i++)
				res = "0" + res;
			return res;
		}
		else{
			int pstv = Integer.parseInt(num);
			String res = Integer.toBinaryString(pstv);
			for(int i = res.length() ; i < 6 ; i++)
					res = "0" + res;
			return res;
